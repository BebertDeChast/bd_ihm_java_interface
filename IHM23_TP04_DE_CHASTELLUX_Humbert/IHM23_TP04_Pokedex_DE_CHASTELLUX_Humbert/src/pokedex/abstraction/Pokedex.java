...
public class Pokedex {
			...
			try (Statement statement = connection.createStatement()) {
				try (ResultSet resultSet = statement.executeQuery(requete)) {
					while (resultSet.next()) {
						...
						this.pokemons.add(new Pokemon(id, nomen, nomfr, typep, types, pv, attaque, defense, attspe,
								defspe, vitesse, generation, legendaire));
					}
				}
			}
		}
	...
	public void goNext() {
		if (this.currentIndex < this.size() - 1) {
			this.currentIndex++;
			this.pcs.firePropertyChange(Pokedex.MESSAGE_CHANGEMENT_POKEMON_COURANT,
					Integer.valueOf(this.currentIndex - 1), Integer.valueOf(this.currentIndex));
		}
	}
	...
	public void goToIndex(int index) {
		if (index >= 0 && index < this.size() && this.currentIndex != index) {
			...
			this.pcs.firePropertyChange(Pokedex.MESSAGE_CHANGEMENT_POKEMON_COURANT, Integer.valueOf(previous),
					Integer.valueOf(this.currentIndex));
		}
	}
	 ...
	 *         pas un entier et il existe un pokemon ayant idOrName pour nom
	 *         (francais ou anglais)
	 */
